...
import com.nashtech.toaitran.exception.CategoryNotFoundException;
...
import com.nashtech.toaitran.service.BaseService;
...
@Service
//@Transactional
public class CategoryService implements BaseService<CateDTO,Category, Long> {
    ...
    public CateDTO findById(Long id) {
        ...
        cate.orElseThrow(()-> new CategoryNotFoundException(id));
        return createFromE(cate.get());
    }

    @Override
    public CateDTO update(Long id,Category category) {
        Optional<Category> cate = categoryRepository.findById(id);
        cate.orElseThrow(()-> new CategoryNotFoundException(id));
        return createFromE(categoryRepository.save(updateEntity(cate.get(),createFromE(category))));
    }

    @Override
    public CateDTO insert(Category category) {
        return createFromE(categoryRepository.save(category));
    }

    @Override
    public CateDTO save(Category category) {
        return createFromE(categoryRepository.save(category));
    }
